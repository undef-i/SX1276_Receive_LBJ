...
std::unique_ptr<data_bond> db = nullptr;
...
void loop() {
    ...
    if (millis64() > 60000 && format_task_timer == 0 && !exec_init_f80) { // Set to 60000 to reduce interference.
...
//        setCpuFrequencyMhz(80);
    ...
    if (pager.available() >= 2 && fd_state == TASK_INIT) {
          ...
          MemoryMonitor::printMemoryInfo("Before creating data_bond");
          db = std::unique_ptr<data_bond>(new data_bond()); // 使用智能指针创建对象
          MemoryMonitor::printMemoryInfo("After creating data_bond");
          runtime_timer = millis64();
          timer4 = millis64();
...
//            Serial.printf("success.\n");
            ...
            // formatDataTask();
            MemoryMonitor::printMemoryInfo("Before creating format task");
...
void handleSerialInput() {
    if (Serial.available()) {
        ...
        if (in == "ping")
            ...
            MemoryMonitor::printMemoryInfo("User request");
        } else if (in == "mem check") {
            bool ok = MemoryMonitor::checkMemoryThreshold(4096);
...
void formatDataTask(void *pVoid) {
    ...
    MemoryMonitor::printMemoryInfo("formatDataTask start");
    bool empty = true;
    for (int i = 0; i < POCDAT_SIZE; i++) {
        ...
        if (i == 0) {
            size_t totalLen = 0;
            for (int j = 0; j < POCDAT_SIZE; j++) {
                if (!db->pocsagData[j].is_empty)
                    totalLen += db->pocsagData[j].str.length() + 10; // 10用于额外格式字符
            }
            db->str.reserve(totalLen + 100);
        }
        db->str += "  " + db->pocsagData[i].str;
    }
    if (empty) {
        fd_state = TASK_DONE;
        task_fd = nullptr;
    ...
    fd_state = TASK_DONE;
    task_fd = nullptr;
    MemoryMonitor::printMemoryInfo("formatDataTask end - before cleanup");
    db.reset(); // 使用智能指针的reset方法释放内存
    MemoryMonitor::printMemoryInfo("formatDataTask end - after cleanup");
    vTaskDelete(nullptr);
}

void simpleFormatTask() { // only output initially phrased data in case of memory shortage
    MemoryMonitor::printMemoryInfo("simpleFormatTask start");
    // 预分配足够的缓冲区避免重复分配
    size_t totalLen = 0;
    for (auto &i: db->pocsagData) {
        if (i.is_empty)
            continue;
        // 预估每个条目的长度
        totalLen += 20 + i.str.length(); // 20用于地址/函数和格式化字符
    }
    db->str.reserve(totalLen + 100); // 额外预留100字节
    ...
    MemoryMonitor::printMemoryInfo("simpleFormatTask end - before cleanup");
    db.reset(); // 使用智能指针的reset方法释放内存
    MemoryMonitor::printMemoryInfo("simpleFormatTask end - after cleanup");
}
// END OF FILE.